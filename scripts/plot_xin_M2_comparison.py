...
def main(args):
    with asdf.open(args.input) as af:
        ax = np.array(af['actuators']['x'])
        ay = np.array(af['actuators']['y'])
    ...
    m2_mat_data = loadmat(args.compare)

    udata = m2_mat_data['Udn3norm'][:, args.imode]*1e6 # m -> um
    vdata = m2_mat_data['Vdn3norm'][:, args.imode] # Newtons
    ...
    colorbar(axes[0,0].scatter(
        m2_mat_data['x'][:, 0], m2_mat_data['y'][:, 0],
        c=udata,
        cmap='seismic',
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input",
        type=str,
        default="M2_NASTRAN.asdf",
        help="Input asdf file.  Default: M2_NASTRAN.asdf"
    )
    parser.add_argument(
        "--compare",
        type=str,
        default= "/Users/josh/src/M2_FEA/data/M2_Udn3norm.mat",
