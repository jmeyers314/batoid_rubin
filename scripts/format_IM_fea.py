...
def main(args):
    ...
    # arr1 = np.load(os.path.join(args.IM_dir, "data", "M1M3", "M1M3_1um_156_grid.npy"))  # Is not PTT-subtracted!
    arr = np.loadtxt(os.path.join(args.IM_dir, "data", "M1M3", "M1M3_1um_156_grid.txt"))  # Is PTT-subtracted!
    m13x = arr[:, 1]
    m13y = arr[:, 2]
    fits.writeto(os.path.join(args.outdir, "M1M3_1um_156_grid.fits.gz"), arr, overwrite=True)
    arr = np.loadtxt(os.path.join(args.IM_dir, "data", "M1M3", "M1M3_1um_156_force.txt"))
    fits.writeto(os.path.join(args.outdir, "M1M3_1um_156_force.fits.gz"), arr, overwrite=True)

    # M1M3_dxdydz_zenith, horizon
    arr = np.load(os.path.join(args.IM_dir, "data", "M1M3", "M1M3_dxdydz_zenith.npy"))
    fits.writeto(os.path.join(args.outdir, "M1M3_dxdydz_zenith.fits.gz"), arr, overwrite=True)
    arr = np.load(os.path.join(args.IM_dir, "data", "M1M3", "M1M3_dxdydz_horizon.npy"))
    fits.writeto(os.path.join(args.outdir, "M1M3_dxdydz_horizon.fits.gz"), arr, overwrite=True)

    # M1M3_thermal_FEA
    arr = np.load(os.path.join(args.IM_dir, "data", "M1M3", "M1M3_thermal_FEA.npy"))
    ...
    for fn in [
        ...
        arr = np.loadtxt(os.path.join(args.IM_dir, "data", "camera", fn), skiprows=1)
